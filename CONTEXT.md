...
**工作目录（Workdir）**：
Codex 进程运行时的仓库根路径；必须是真实存在的目录，且路径字符串不含字面引号（带引号入参是 Windows os error 123 的元凶）。中文 / 非 ASCII 路径直接传裸路径，归一后原样交给 Codex，不做任何别名改写。
_Avoid_: cwd 与业务「当前 shell 目录」混用（以传入的工作目录为准）；给 cd 包字面引号
