...
**工作目录（Workdir）**：
Codex 进程运行时的仓库根路径；必须是真实存在的目录，且路径字符串不含字面引号。Windows 上若路径含非 ASCII（中文等），包装层可改用 ASCII 目录联接别名再交给 Codex，真实仓库位置不变。
_Avoid_: cwd 与业务「当前 shell 目录」混用（以传入的工作目录为准）；把联接缓存目录当用户数据
