...
**工作目录（Workdir）**：
Codex 进程运行时的仓库根路径；必须是真实存在的目录，且路径字符串不含字面引号。Windows 上若路径含非 ASCII（中文等），包装层可改用 ASCII 目录联接别名再交给 Codex，真实仓库位置不变。有两个不可混用的所指，分别见下两条。
_Avoid_: cwd 与业务「当前 shell 目录」混用（以传入的工作目录为准）；把联接缓存目录当用户数据；不加限定词的 workdir 同时指两个所指

**真实仓库路径（Real workdir）**：
归一后的用户仓库位置；领域结局（ReviewResult）对外报告一律用它。
_Avoid_: 把审核别名当真实仓库路径报告给用户

**审核别名（Codex workdir）**：
实际交给 Codex 的工作目录——Windows 非 ASCII 路径下为 ASCII 目录联接别名，否则等于真实仓库路径；经行流 seam 的 workdir= 参数传给 runner。
_Avoid_: 对别名 resolve() 跟随联接回到真实路径后再交给 Codex；把别名写进对外报告
