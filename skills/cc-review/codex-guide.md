...
场景表以 [SKILL.md](SKILL.md) 的 SESSION_ID 表为唯一来源：初审 `""`；复审带上一轮
返回值；会话丢失则 `""` 重建并在 PROMPT 附初审摘要。失败返回也带 `SESSION_ID`
（未建会话为 null）——非 null 时复审直接复用，无需重建。
...
**元凶是字面引号**：`cd = "\"C:/Users/you/repo\""` 这类带引号入参会被 Windows
判为非法路径（os error 123），旧版曾因此误报 `protocol_missing_session`。
包装层已剥包裹引号 / `file://` / 弯引号（normalize_workdir），调用方仍应传裸路径。

**中文 / 非 ASCII 路径无需特殊处理**：直接传裸路径（`C:/Users/you/中文路径/repo`），
包装层原样交给 Codex——实测当前 Codex CLI 在非 ASCII 工作目录下内部工具工作正常。
若个别环境仍遇 123，把仓库放到纯英文路径即可绕过。

若报 `invalid_path`：检查目录是否存在、是否仍含引号。该种类只会出现在**未取得
SESSION_ID** 的运行上；已建会话后，子工具输出里的 os error 123 文本只计入诊断
（`json_decode_errors` / `last_lines`），不会盖掉成功审查。
...
5. 只读沙箱通常**起不了全量测试**（无可写临时 / 缓存目录）——自测证据由 Claude
   写进 PROMPT；需要 Codex 亲验时给定向单测命令（如 pytest 加 `-p no:cacheprovider -s`）
