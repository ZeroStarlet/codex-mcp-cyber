...
SCCG 通过连接多个顶级模型，构建了一个高效、低成本且高质量的代码生成与审核流水线：
...
**[单元测试批量生成](cases/2025-01-05-unit-test-generation/README.md)** - SCCG 架构实测记录

| 指标 | 纯 Claude 方案 | SCCG 协作方案 | 说明 |
| :--- | :--- | :--- | :--- |
...
3. 调用 coder (或 gemini) 工具 → 执行代码生成/修改
       ↓
4. Claude 快检，通过后**必须立即主动**调用 Codex 审核
       ↓
5. 调用 codex 工具 → **必须**独立 Code Review
       ↓
6. 根据审核结论：✅ 通过 → 继续；⚠️/❌ 有问题 → 委托 Coder 修复 → 再审 → 循环直至所有问题关闭
```
...
- 配置文件保存在 `~/.sccg-mcp/config.toml`，权限设置为仅当前用户可读写
...
[CC Switch](https://github.com/farion1231/cc-switch) 是一款跨平台桌面工具，提供统一的图形化界面来管理 Claude Code、Codex 及 Gemini CLI，无需手动编辑配置文件。
...
- 填写以下信息：
  ...
  - **Args**：
    ...
    ```
  - **Transport**：`stdio`
- 勾选需要使用 SCCG 的应用（Claude Code、Codex、Gemini CLI）
- 点击保存，CC Switch 会自动同步配置到选中的应用

**2. 配置 Coder 后端 API**

SCCG 的 Coder 工具需要配置后端模型 API。请参考前面的"配置 Coder"章节，手动创建 `~/.sccg-mcp/config.toml` 文件并填写 API Token、Base URL 和 Model。

> 💡 **说明**：CC Switch 目前不直接管理 SCCG 的 Coder 配置，需要手动配置。

**3. 安装 Skills（可选）**

- 方式一：在 CC Switch 的 Skills 管理器中，添加本项目的 GitHub 仓库 `https://github.com/ZeroStarlet/Sisyphus-Coder-Codex-Gemini.git`，选择 `skills/sccg-workflow` 和 `skills/gemini-collaboration` 进行安装
- 方式二：手动复制本项目的 `skills/` 目录到 `~/.claude/skills/`

**4. 配置全局 Prompt（推荐）**

- 在 CC Switch 的 Prompts 管理器中创建新预设
- 将本项目 `templates/sccg-global-prompt.md` 的内容复制到预设中
- 激活该预设，CC Switch 会自动同步到 `~/.claude/CLAUDE.md`

**5. 验证配置**

重启 Claude Code CLI，运行 `claude mcp list` 检查 SCCG 是否已连接。
...
在 Windows 上使用 SCCG-MCP，请确保以下 CLI 工具已正确添加到系统 PATH：
...
- **强制审查**：每次修改完成后**必须立即主动**委托 Codex 执行代码审查，**得到 ✅ 通过 信号后方可合并 / 提交 / 推送**
- **问题闭环**：审查（Codex 或人工）登记的每一个问题项，必须在**当前修改轮次内**修复，并反馈至审查方确认关闭
- **零遗留**：所有问题在**项目跟踪记录**（审查回复 / TaskList / issue tracker 任一载体）中必须到达"关闭"状态。**绝对禁止**遗留任何未解决或被忽略的问题项
...
**在调用任何 SCCG MCP 工具之前，必须先执行对应的 Skill 获取最佳实践指导：**

| MCP 工具 | 前置 Skill | 执行方式 |
|----------|-----------|---------|
| `mcp__sccg__coder` | `/sccg-workflow` | 必须先执行 |
| `mcp__sccg__codex` | `/sccg-workflow` | 必须先执行 |
| `mcp__sccg__gemini` | `/gemini-collaboration` | 必须先执行 |

**执行流程**：
1. 用户请求使用 Coder/Codex/Gemini
2. **立即执行对应 Skill**（如 `/sccg-workflow`、`/gemini-collaboration`）
3. 阅读 Skill 返回的指导内容
4. 按照指导调用 MCP 工具
...
| **Codex** | 代码审核者/高阶顾问 | 质量把关、Review（**每次修改完成后必须立即审核**） | read-only | 默认 1 次 |
...
4. **Codex 审核**：每次修改完成后**必须立即主动**调用 Codex 审核
